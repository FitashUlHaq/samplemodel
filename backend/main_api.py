...
async def create_book(book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    db_book = Book(
        stock=book_data.stock,        title=book_data.title,        pages=book_data.pages,        price=book_data.price,        release=book_data.release,        time=book_data.time        )
...
async def bulk_create_book(items: list[BookCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_book = Book(
                stock=item_data.stock,                title=item_data.title,                pages=item_data.pages,                price=item_data.price,                release=item_data.release,                time=item_data.time            )
            database.add(db_book)
            database.flush()  # Get ID without committing
